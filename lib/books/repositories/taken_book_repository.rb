...
class TakenBookRepository
  ...
  def find_last_by_username_and_book_id(username, book_id)
    book = @book_repository.find_by_id(book_id)
    user = @user_repository.find_by_username(username)

    TakenBook.where({ book: book, user: user }).first
  end

  def find_last_by_book_id(book_id)
    book = @book_repository.find_by_id(book_id)

    TakenBook.where({ book: book }).first
  end

  def save_new_taken_book(username, book_id)
    user =  @user_repository.find_by_username(username)
    book = @book_repository.find_by_id(book_id)
